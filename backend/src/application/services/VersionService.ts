...
export class VersionService {
  ...
  public async listVersions(familyId: string): Promise<FamilyVersion[]> {
    return prisma.familyVersion.findMany({ where: { familyId }, orderBy: { versionNumber: 'desc' } });
  }

  public async rollbackToVersion(familyId: string, versionNumber: number, actorId: string): Promise<FamilyVersion> {
        ...
        for (const person of snapshot.persons) {
          ...
          if (!identityId) {
            ...
            const [emailIdentity, phoneIdentity] = await Promise.all([
              email ? tx.personIdentity.findUnique({ where: { email }, select: { id: true } }) : Promise.resolve(null),
              phone ? tx.personIdentity.findUnique({ where: { phone }, select: { id: true } }) : Promise.resolve(null),
            ]);
            if (emailIdentity && phoneIdentity && emailIdentity.id !== phoneIdentity.id) {
              throw new Error('Snapshot identity conflict: email and phone map to different identities');
            }
